// src/components/TodoDateFilter.jsx
import { Suspense, useEffect, useState } from 'react';
import toast from 'react-hot-toast';

export default function TodoDateFilter({
	startDate,
	endDate,
	today,
	setStartDateWithSwap,
	setEndDateWithSwap,
	swapTrigger,
	resetPage, // ← new prop
}) {
	const [Framer, setFramer] = useState(null);

	useEffect(() => {
		import('framer-motion').then((mod) => setFramer(mod));
	}, []);

	if (!Framer) return null;

	const { motion, AnimatePresence } = Framer;

	return (
		<Suspense fallback={null}>
			<AnimatePresence mode='wait'>
				<motion.div
					key={swapTrigger}
					initial={{ opacity: 0, backgroundColor: '#dbeafe' }}
					animate={{ opacity: 1, backgroundColor: '#ffffff' }}
					exit={{ opacity: 0 }}
					transition={{ duration: 0.35 }}
					className='flex flex-wrap gap-3 mb-8 items-center'
				>
					<input
						type='date'
						value={startDate}
						onChange={(e) => setStartDateWithSwap(e.target.value)} // ← no reset here
						max={endDate || today}
						className='border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 transition'
					/>
					<input
						type='date'
						value={endDate}
						onChange={(e) => setEndDateWithSwap(e.target.value)} // ← no reset here
						min={startDate}
						max={today}
						className='border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 transition'
					/>
					<button
						onClick={() => {
							setStartDateWithSwap(''); // ← clear via hook
							setEndDateWithSwap('');
							resetPage(); // ← only reset page
							toast.success('Date filters cleared');
						}}
						className='px-5 py-2 bg-gray-100 rounded-xl text-sm hover:bg-gray-200 transition font-medium'
					>
						Clear Dates
					</button>
				</motion.div>
			</AnimatePresence>
		</Suspense>
	);
}
